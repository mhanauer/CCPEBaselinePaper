...
Plot.SD<-ggplot(data=Cig.SD, aes(x=CenterRSKCIG, y=fit, group=CenterREL_IMP))+
  ...
  xlab("Centered Risk Cigarettes")+ #Adds a label to the x-axis
...
```
Cig Subgroup Anlaysis
```{r}
CCPEBaseline = data.frame(na.omit(CCPEBaseline))
dim(CCPEBaseline)

CCPEBaselineR1 = subset(CCPEBaseline, REL_IMP ==1) 
dim(CCPEBaselineR1)

cigR1 = glm.nb(CIG30D ~ RSKCIG + R_WHITE_N  + AGE  + INCOME + SEX_PR + GENDER , data = CCPEBaselineR1)
summary(cigR1)

CCPEBaselineR2 = subset(CCPEBaseline, REL_IMP ==2) 
dim(CCPEBaselineR2)

cigR2 = glm.nb(CIG30D ~ RSKCIG + R_WHITE_N  + AGE  + INCOME + SEX_PR + GENDER , data = CCPEBaselineR2)
summary(cigR2)


CCPEBaselineR3 = subset(CCPEBaseline, REL_IMP ==3) 
dim(CCPEBaselineR3)

cigR3 = glm.nb(CIG30D ~ RSKCIG + R_WHITE_N  + AGE  + INCOME + SEX_PR + GENDER , data = CCPEBaselineR3)
summary(cigR3)

CCPEBaselineR4 = subset(CCPEBaseline, REL_IMP ==4) 
dim(CCPEBaselineR4)

CCPEBaselineR4$CIG30D

cigR4 = glm.nb(CIG30D ~ RSKCIG + R_WHITE_N  + AGE  + INCOME + SEX_PR + GENDER , data = CCPEBaselineR4)
summary(cigR4)
...
CCPEBaselineMarin$INCOME = factor(CCPEBaselineMarin$INCOME, level =c(1,0), labels = c("Low", "High"))
...
Plot.MarIncome<-ggplot(data=Mar.SD, aes(x=CenterRSKMJ, y=fit, group=INCOME))+
  ...
  ylab("Predicted value of marijuana use")+
  xlab("CenterRSKMJ")+
  ggtitle("Interaction bewteen risk of marijuana use and income")+
  theme_bw()+ 
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank())+
  scale_fill_grey()
...
Plot.MarIncome<-ggplot(data=MarGender.SD , aes(x=CenterRSKMJ, y=fit, group=GENDER))+
  ...
  ylab("Predicted value of marijuana use")+
  xlab("CenterRSKMJ")+
  ggtitle("Interaction bewteen risk of marijuana use and gender")+
  theme_bw()+ 
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank())+
  scale_fill_grey()
...
Subgroup risk and gender for mar
